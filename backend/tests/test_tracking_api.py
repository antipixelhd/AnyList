...
class TrackingApiTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_title_and_season_tenths_and_movie_season_rejection(self):
        ...
        season = await self.save(self.show,rating_mode='average',season_scores={'1':7.2})
        self.assertEqual(season.status_code, 200, season.text)
        self.assertEqual(season.json()['season_scores']['1'], 7.2)
        self.assertEqual((await self.save(self.show,season_scores={'1':7.25})).status_code,422)
    ...
    async def test_public_people_search_and_one_way_follow(self):
        ...
        follow=await self.client.post(f'/tracking/people/{self.friend.username}/follow')
        self.assertEqual(follow.status_code,200,follow.text)
        owner_person=await self.client.get(f'/tracking/people/{self.owner.username}')
        self.assertEqual(owner_person.status_code,200,owner_person.text)
        friend_card=next(person for person in owner_person.json()['people'] if person['username']==self.friend.username)
        self.assertEqual(friend_card['id'],self.friend.id)
        self.assertTrue(friend_card['has_avatar'])
        self.assertTrue(friend_card['following'])
