...
Notifications now uses Home's two-column rhythm: notification cards and pending deliveries on the left, with Inbox preferences, ignored imports, and history import visible in a compact right rail. On phones the tools follow the cards. Posters retain a fixed size and sit vertically centered in each card; linked titles use the blue accent, rating-request cards omit the redundant completion sentence, and every Rate now control uses the shared yellow treatment. Needs review uses the separate rose alert accent.
