...
Daily cards show only the most important change for that person/title/UTC day. Priority is Completed/Dropped/Paused/Plans to watch, watched season, watched episode range, Started Watching, then Rated. Season and episode actions apply only to series progress changes; movie cards never describe episodes. A rating remains visible as the chip on any higher-priority card and is never repeated in the action sentence. The action text is light and the linked title is blue without a hover color change. Later same-day changes replace and raise the existing card even when other titles were updated between them. Home keeps its server-rendered initial feed and incrementally merges cursor updates every 60 seconds while visible and on focus; this refreshes activity already recorded by sync but does not trigger provider synchronization.

Movie list progress follows the current status: Completed is 1; Watching, Paused, Dropped, and Planning are 0. Status edits preserve recorded viewing history. A one-time migration repairs existing movie rows that carried stale progress from a former status.
...
The app-bar badges, Home's Notifications summary, the nonmodal AttentionPrompt, and the Notifications page share one in-memory full-snapshot refresh. The initial server-rendered state remains authoritative for a normal load; after hydration, the snapshot uses no-store reads immediately, every 60 seconds while visible, and on tab focus/visibility return. Hidden pages pause, Astro navigation and account changes clean up the lifecycle, and successful local mutations trigger an immediate refresh. Reconcile additions, replacements, reordering, and removals from the full snapshot, while preserving a focused or dirty search/status/rating interaction until it ends. Do not persist private notification content in localStorage; the prompt may retain account-scoped opaque acknowledgment keys so it stays dismissed across browser visits without changing inbox cards. Opening Notifications acknowledges prompts present while that page is active, but later arrivals after leaving remain eligible. The outbound connection-delivery queue is separate from the unread app-bar badge. Home activity uses its separate cursor-based incremental refresh; Profile Overview remains SSR-only.
...
Notifications now uses Home's two-column rhythm: notification cards and pending deliveries on the left, with Inbox preferences, ignored imports, and history import visible in a compact right rail. On phones the tools follow the cards. Rating-request posters fill their card height; other posters retain a fixed size and sit vertically centered. Linked titles use the blue accent, rating-request cards omit the redundant completion sentence, and every Rate now control uses the shared yellow treatment. Needs review uses the separate rose alert accent. A status-review card shows provider and badge, title, status transition, then Confirm, inline Status selection, and Keep previous. Confirm uses the selected status when changed; the redundant Apply and Edit actions and repeated status explanation are omitted. Other review cards keep their decisions specific to their event kind.
