...
The movie review threshold now accepts a same-title TMDB result when its vote count and popularity clearly dominate other exact-title films released before the Netflix watch. This resolves *The Hangover*, *Parasite*, and *Dune* in the reference export; close remakes still require review. The matcher also searches colon-bearing show titles such as *Star Wars: The Clone Wars*, checks episode titles containing colons, and accepts an exact show identity independently of its unresolved episodes. The 469-row reference now yields 57 automatically accepted title identities, 13 named shows requiring title review, and 16 rows whose Netflix show name is missing. Of the 65 unresolved episode rows, 16 are those unnamed rows. They remain visible for manual mapping or skipping. No personal import was committed during this check.

Settings → Connections → Import now includes a Netflix guide and CSV upload. The importer parses viewing dates, deduplicates rows, matches titles and episode names against TMDB with bounded requests, and prepares a private review draft. Uncertain titles and episodes require confirmation, remapping, or skipping. The progress review defaults incomplete shows to Partial and offers cumulative endpoint customization; the final Import writes watched events and tracked entries in one database transaction. Existing progress is not reduced, repeated watches are deduplicated, and cancel discards a draft. The new `mt024` migration stores expiring draft sessions.

The supplied 469-row sample parses without errors and now prepares against the local TMDB configuration. Netflix's second part of *Fate/Apocrypha* has no TMDB season 2 endpoint; exact episode-title matching maps 24 of its 25 source episodes into TMDB's available season, leaving one for review. A missing TMDB season no longer aborts the CSV. The `mt024` migration and 25 focused importer tests pass against a disposable local PostgreSQL database; the frontend production build passes. The full backend run completed 1,264 tests with the same 10 errors in existing auth, history, and RPDB test fakes outside the importer. Authenticated browser review remains open because the local preview redirects to login. No VPS deployment or production import was performed.
