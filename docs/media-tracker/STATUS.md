...
Final local walkthrough checked compact lists at 2560×1440, 3840×2160 and 390×844, Browse at 390×844 and 320×720, mobile detail/stats, misspelled as-you-type search, and an unauthenticated public profile with stats. No horizontal overflow appeared on those routes. It found and fixed detail-rating accessibility, anonymous rating, fast-search control, phone navigation/scroll, Browse recovery, and phone filter focus defects. The Astro build and local authenticated route verifier passed; disposable score changes were restored. Commit `09af96a` is public and deployed to the isolated test instance after a private validated database backup. Its exact public CI run passed both jobs. The app and retained database are healthy at `mt014`, the running app reports `09af96a`, login/PWA/OIDC-entry HTTPS checks passed, and read-only SQL confirms both disposable Stremio/Nuvio connections still have playback and watched pushing disabled. The local disposable account password is not valid on the isolated instance; Google OIDC remains the intended isolated sign-in path for the final authenticated check. These browser checks do not substitute for real Android Chrome or iPhone Safari acceptance.
