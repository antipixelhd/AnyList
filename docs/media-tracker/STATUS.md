...
Final local walkthrough checked compact lists at 2560×1440, 3840×2160 and 390×844, Browse at 390×844 and 320×720, mobile detail/stats, misspelled as-you-type search, and an unauthenticated public profile with stats. No horizontal overflow appeared on those routes. It found two detail-rating defects: anonymous visitors were offered a quick-rate button they could not use, and after a saved score the button still announced the previous score. The anonymous control now links to sign-in with a return path; quick-rate saves update its accessible name with the visible score. The Astro build passed, a disposable 8.5/10 half-point save announced 8.5/10, and its original unrated state was restored. Commit `30f5da6` is public and deployed to the isolated test instance; the app and retained database are healthy and the running app reports that exact version. Public login, manifest and service-worker checks passed. The local disposable account password is not valid on the isolated instance; Google OIDC remains the intended isolated sign-in path for the final authenticated check. These browser checks do not substitute for real Android Chrome or iPhone Safari acceptance.
