...
The supplied 469-row sample parses without errors and now prepares against the local TMDB configuration. Netflix's second part of *Fate/Apocrypha* has no TMDB season 2 endpoint; exact episode-title matching maps 24 of its 25 source episodes into TMDB's available season, leaving one for review. A missing TMDB season no longer aborts the CSV. The `mt024` migration and 18 focused importer tests pass against a disposable local PostgreSQL database; the frontend production build passes. The full backend run completed 1,257 tests with the same 10 errors in existing auth, history, and RPDB test fakes outside the importer. Authenticated browser review remains open because the local preview redirects to login. No VPS deployment or production import was performed.
