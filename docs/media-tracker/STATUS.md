...
OIDC remains generic but now has a secure Google configuration: enabled flows fail closed when required values are missing, optional verified-email enforcement rejects unverified claims, matching is case-insensitive against an existing administrator-provisioned email, and unknown identities are not auto-created. The administration form can create an SSO-only user without inventing a password; supplied passwords continue to work as a fallback. `GOOGLE-SSO.md`, `.env.example`, and `docker-compose.yaml` contain the current Google endpoints and exact callback guidance. The isolated deployment uses `https://media-tracker-test.don-cloud.dedyn.io/oidc-callback` and requests only `openid email`.
...
The isolated `media-tracker-test` Compose project is now running on the VPS at `http://100.106.207.96:8460`. The app image rebuild normalized Windows line endings in `/entrypoint.sh`, resolving the previous container restart-127 failure. The existing database volume was preserved; migrations completed successfully through `mt006`, the app container reports healthy, and the bound `/health` endpoint responds successfully.

The VPS-wide verification showed the existing Scrob, Comet, AIOStreams, gateway, dashboard, and supporting projects still running. Only the isolated `media-tracker-test` app service was rebuilt/recreated; its PostgreSQL container remained healthy and was not recreated. Temporary transfer archives were removed from both the VPS and the local workspace. This deployment remains a test instance: real outbound provider writes and Google OAuth client verification are still disabled.
...
Committed build `83a4fca` was archived without local credentials and deployed over the existing `/opt/media-tracker-test` source. Only `media-tracker-test-app-1` was rebuilt and recreated; the existing PostgreSQL container and both named volumes were preserved. The initial update migrated the retained database from `mt006` through `mt007` and `mt008`; the current build adds TVDB-native streaming observations, the Media Tracker authentication shell, completed release-facing product identity, and Media Tracker export/backup/webhook labels. The app and database report healthy, the database remains at `mt008`, and the served manifest/source marker identify Media Tracker.

At this deployment checkpoint, the VPS Compose inventory confirmed Scrob, Comet, AIOStreams, AIOmetaData, public gateway, dashboards, and the remaining unrelated projects stayed running. Temporary local and VPS transfer archives were removed. The isolated database initially contained zero users and zero media-server connections, so no provider state could be touched accidentally. Later checkpoints below record disposable provider provisioning, controlled outbound verification, and the dedicated Google OIDC test host.
...
Commit `a5939f9` is deployed to `/opt/media-tracker-test`. Only the isolated app container was rebuilt; its database and application volumes were preserved and migrated from `mt008` through `mt009` to `mt010`. The app and database are healthy at `http://100.106.207.96:8460`, the source marker reports `a5939f9`, and both disposable Stremio and Nuvio connections retain `push_playback=false` and `push_watched=false`. The VPS container inventory confirms Scrob, Comet, AIOStreams, AIOmetaData, the public gateway, and all other neighboring projects remained running.

The isolated instance is also available at `https://media-tracker-test.don-cloud.dedyn.io` through a dedicated public-gateway route and Let's Encrypt certificate. The deSEC updater now maintains that hostname. OIDC is enabled with the exact registered callback, verified-email enforcement, disabled self-registration and account auto-creation, and password login retained. The `provider-test` administrator has a confirmed email match and a password hash, preserving recovery. Both the new login and existing Scrob login return HTTP 200, and the real Google authorization page receives the exact callback with `openid email`. Interactive Google sign-in successfully returned to Media Tracker and selected the existing `provider-test` account, closing the OIDC deployment gate.
...
Commit `4a7f69a` is deployed to the isolated public test instance. Only `media-tracker-test-app-1` was rebuilt and recreated; the retained PostgreSQL container remains healthy at `mt010`. The public login returns HTTP 200, Google authorization still emits the exact registered callback with `openid email`, and the source marker reports `4a7f69a`. Scrob, Comet, AIOStreams, AIOmetaData, the public gateway, dashboards, and the other VPS projects remained running. The transfer archive was removed after deployment and a pre-update source backup remains on the VPS. `RELEASE-AUDIT.md` maps the accepted first-release scope to its implementation and verification evidence.
...
Commit `03a8277` is deployed to the isolated public test instance. Only `media-tracker-test-app-1` was rebuilt and recreated; its database and application-data volumes were preserved and migrated from `mt010` to `mt011`. The app and database are healthy, the source marker reports `03a8277`, login, manifest, and service-worker routes return successfully, the deployed worker contains the rating push handler, and the protected push route redirects anonymous callers to authentication. Google OIDC still enters through the public callback flow. Scrob, Comet, AIOStreams, AIOmetaData, the public gateway, and all other neighboring VPS projects remained running. Transfer artifacts were removed; the pre-update source backup remains on the VPS.
