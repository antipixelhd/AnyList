...
## Stage Two container release and VPS rollout (2026-09-25)

The owner completed the local manual review and authorized VPS rollout. Commit `d90a663` passed public backend/frontend CI. A fresh local PostgreSQL database migrated from base to `mt025`, and all 1,307 backend tests passed; the Astro production build and local production Docker build passed. The manual multi-architecture workflow published `ghcr.io/antipixelhd/anylist:latest` for AMD64 and ARM64 from that commit. The VPS is ARM64; the previous x64-only workflow was removed. The workflow publishes a commit-specific tag alongside `latest`, and deployment Compose files and secrets remain on the VPS outside Git.

The isolated test database was backed up and its dump validated before replacing only its app container with the published image. Its database and app volumes were retained, migrations reached `mt025`, and the app is healthy. The test login page, signed-out redirect, assets, and internal OIDC/registration settings passed read-only checks. The former source-build Compose file is retained on the VPS for rollback. Neighboring Caddy, Scrob, and AIOStreams services remained running.

A separate production Compose project now has fresh database and app volumes. Its image is healthy at `mt025`; the owner registered the first password-backed account, which has both the admin role and flag. Normal registration then closed. Google OIDC is enabled with verified-email account creation, password login remains enabled, and anonymous navigation is disabled. Caddy serves `anylist.don-cloud.dedyn.io` over HTTPS through an internal Docker alias. A temporary owner-network IP gate protected the initial registration and was removed after the admin account was verified. From outside that former gate, login returned 200, registration redirected, and a signed-out app page redirected to login. The production Google authorization request used the exact registered callback and `openid email` scope. The deSEC updater includes the new hostname. No provider outbound writes were enabled or performed during rollout.

The owner completed interactive production Google sign-in to the existing administrator account; the database still contained one user. Remaining interactive checks: sign in with a new verified Google identity, confirm that account is non-admin, and repeat the release acceptance paths on the deployed desktop and physical phones. The earlier optional season-completion rating prompt and separate tracked-list season rows remain outside this rollout; current season ratings and release notices are the accepted release behavior.
...
Movie, series, and combined profile lists keep the two-button view switch inside list content, floating above the first section 20px below the profile tabs and aligned with the tables' right edge. It adds no layout row, so the tables start higher. The filter column begins at the shared 32px desktop/24px phone profile-content gap; the table column sits beside it and starts slightly lower through the status heading's 18.72px top margin. Status headings also have 30px left padding. AGENT.md records the view-switch exception. Frontend production build and focused importer/projection unit tests pass. A database-backed multi-season importer-progress regression was added but could not run locally because no disposable PostgreSQL test database was available. Desktop and phone browser checks confirmed the view-switch offset, alignment, and no horizontal overflow.
...
Settings → Connections → Import includes a Netflix guide and CSV upload. The importer opens a wide dialog after upload, then moves through title review, two-column show progress, and final review with Continue and Back. It parses viewing dates, deduplicates rows, matches titles and episode names against TMDB with bounded requests, and prepares a private review draft. Uncertain title identities still require confirmation, remapping, or skipping. Episode positions are resolved automatically from exact matches and CSV viewing order, with read-only per-show details. The progress review defaults incomplete shows to Partial, offers cumulative endpoint customization, and allows Completed whenever released season counts are known, even if episode records or air dates are missing. Such backfilled positions become provisional watches. The final review displays only the selected movies, shows, and episode-position totals; Import writes watched events and tracked entries in one database transaction. Existing progress is not reduced, repeated watches are deduplicated, and cancel discards a draft. The `mt024` migration stores expiring draft sessions.

The supplied 469-row sample parses without errors and prepares against the local TMDB configuration. In the latest read-only smoke check, 16 titleless rows were excluded, 41 episode positions were guessed, 9 observations were discarded, 1 bounded observation was covered, and no episode required user review. Fifteen title identities remained for review. A missing TMDB season no longer aborts the CSV. The focused importer tests and frontend production build pass against the disposable local setup. No VPS deployment or production import was performed.
...
## Final local density and typography pass (2026-09-21)

Home, Browse, the combined Profile list, Stats and Title details were reviewed together after the list, profile-header, app-bar and Stats changes. At 1440×900 the shared app bar, profile gradient/navigation, left filter rail, 52px compact rows, icon-led Stats summary and detail hierarchy use the same spacing and type scale. At 390×844 the two-level mobile header, profile tabs, filter disclosure, 44px list artwork, Stats controls/ledger and title actions remain readable and preserve their intended order.

The same five canonical surfaces measured exactly to the viewport with no horizontal overflow at 320×720. At 3840×2160 the complete seven-row synthetic list remained bounded by the desktop content grid and all three Stats charts rendered. The production Astro build passes. Reference screenshots are `verification/final-home-desktop.png`, `final-list-desktop.png`, `final-stats-desktop.png`, `final-title-desktop.png` and their `-phone` counterparts. Physical Android Chrome and iPhone Safari, dense real-account data and final deployed keyboard/touch acceptance remain release gates.

## Final local and public automated gate (2026-09-21)
...
## List progress and daily activity corrections (2026-09-23)

The combined list displays Movie/Series with the same size and weight as progress, including on phones. Its progress increment control is a plus and updates the row, status group, filter counts, and season position in place. Daily series activity records only net forward episodes from the day's starting progress. Correcting progress backward shrinks the existing daily episode range or removes a progress-only card when the net gain reaches zero; an independent rating or status event remains. Episode ranges name their season, and completed-season details include only seasons crossed by that day's progress. Provider observations and legacy history writes use the same forward-progress handling.

Verification used a separate migrated PostgreSQL database: 97 tracking and legacy-history tests passed, including S5E8 → S5E6 → S5E8 → S5E7 → S5E6. The frontend production build passed. Desktop and 390px browser checks confirmed matching Type/Progress typography, a visible plus and Type column, no phone overflow, and an increment from S2E1 to S2E2 without a document reload. The synthetic preview entry was restored after QA; no provider or VPS state was changed.
...
## Upcoming seasons and release inbox (2026-09-25)

Home now includes Upcoming releases below Series in Progress. It lists a tracked series when its next regular season has a confirmed premiere date, uses season-specific artwork when available, and falls back to the show poster while leaving the artwork eligible for later metadata refresh. Hover reveals the title, season number, release date, and day-level countdown. The Series in Progress hover panel is hidden immediately at rest, while the airing card keeps its episode, countdown, and catch-up indicator.

Inbox preferences now include default-on New Season release dates and New Season releases toggles. A confirmed future season date creates a dismissible date notice for an entry currently in Watching, Completed, or Planning; its premiere creates a separate notice with Watching and Planning actions. These notices affect the notification count but never open the attention prompt. Per-user season observations and event keys prevent repeated notices after dismissal, handle changed dates, and let season artwork refresh on still-visible cards. The daily metadata sweep checks TMDB and TVDB season summaries; manual metadata refresh also checks for notices. Seasons without confirmed dates and a show's first season do not qualify.

The frontend production build, backend import/compile checks, migration-head check, and 11 focused season logic tests passed. PostgreSQL-backed route tests and browser review remain for an environment with a disposable test database and signed-in preview.
