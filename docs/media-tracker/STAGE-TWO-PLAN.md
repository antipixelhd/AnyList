...
Status: the additional owner-approved Stage Two requirements were recorded on 2026-09-21 and implemented locally on 2026-09-22. The next step is owner manual verification before any VPS deployment or verification. The earlier implementation evidence below remains historical; see STAGE-TWO-REVIEW.md for the initial evidence.

The per-page loading and transition design is in [STAGE-TWO-LOADING-TRANSITIONS-DESIGN.md](STAGE-TWO-LOADING-TRANSITIONS-DESIGN.md). The implementation and local evidence are recorded in the handoff section below.

## Additional owner acceptance contract (2026-09-21)

These are mandatory Stage Two gates before any verification on the VPS. They supersede conflicting earlier progress notes and preferences in this document, especially the textless-poster preference, dark-text CTA hover, connection-only Library action, and visible push-interval settings. Local implementation does not establish deployed or owner acceptance.

### Local-first Library and connections

- Streaming Library membership is an AnyList-owned choice, separate from Favorite and tracked-list membership. Add/remove must save locally even with no connected service; later eligible Stremio/Nuvio connections receive the desired membership after their required first-import reconciliation. A title remains visible in the owner's Library without a connection.
- The title Library button reflects the saved local choice immediately without reloading the page or flashing other UI. Remove the surrounding “Streaming library,” per-connection state, and “Library push off” explanatory clutter; no success or pending indicator sits beside the button. A local database save failure is shown at the action. Provider delivery failures and retry details belong in Notifications, not beside the button; a local save must never be falsely reported as remote delivery.
- Begin eligible outbound delivery promptly after the local save, outside the interaction's blocking path. Serialize/coalesce per-destination work to prevent collisions, retain durable partial-failure state and retries, and preserve reviewed-baseline/outbound-opt-in gates. A hidden scheduled recovery pass may retry failures; it is not a reason to hold ordinary changes for a long interval. Manual push to a selected provider remains available.
- Different AnyList users may independently connect the same external account or Nuvio profile using their own login sessions. Do not impose an exclusive-account guard or a special one-writer restriction. This does not waive ordinary per-connection credential, conflict, and privacy safeguards, and does not make one user's local data another user's data.

### Reconcile first, then targeted propagation

- Keep each connection's configured pull interval. When pulls overlap, gather their available complete snapshots in one bounded cycle, compare each against its own approved baseline, determine deltas and resolve compatible changes once, apply the resulting AnyList state, then push only affected fields to eligible destinations other than the source of the change. A shorter-interval source can propagate its changes without waiting for another source's next pull; overlapping pulls at the same interval must be deduplicated together. Manual full sync may request all enabled sources.
- A missing, failed, sparse, or incomplete source is not deletion evidence and must not block unrelated safe changes indefinitely. Preserve unresolved source state for later reconciliation. After repeated failures create/update one persistent provider/connection Notification with a **Resolve** link into the relevant Connection Settings; actionable authentication or permission failures notify immediately. Automatically clear it on recovery. Ordinary loss of the user's internet connection is handled by the offline UI, not a provider-failure Notification.
- Compare baselines to establish that a change occurred before choosing precedence. Use the latest *trustworthy, comparable modification time* for competing new observations, even when the newer playback position is lower. Retain Stremio item modification evidence; investigate Nuvio's playback time and ordered delta events, and track outbound echoes/acknowledgments. Nuvio `last_watched` is not a universal modification timestamp, and provider clocks or deletion events may be incomparable. Do not rank one provider above another. When order genuinely cannot be established, retain the local value and require review rather than guessing or fanning out a conflict.
- An explicit local watch/playback correction beats proven stale provider state and propagates the correction without review. For genuinely new, equally trustworthy simultaneous observations, choose the furthest completed regular episode for series watch state and the furthest in-title playback position for movies/episodes. Preserve local history. First-connection reconciliation approval and the existing field-specific confirmation gates still precede outbound writes.
- Local tracking changes and reconciled inbound deltas should trigger prompt, targeted outbound work where destination APIs support it. Include actual current movie/episode identity, playback position and observation time when projecting resume state to Stremio/Nuvio; verify each provider's capability rather than equating watched state with resume position. Users keep their existing per-connection choices of which fields to pull/push. Do not expose an automatic **Push interval** control; enabled push fields make automatic delivery active. Keep manual Push.

### Lists, editor, and touch interaction

- Lists show the furthest completed regular episode coordinate (`S1E10`, for example) for series, not the most recently replayed episode. Movie watch progress displays `0` or `1`; `1` means watched. The owner-facing increment control appears for movies and series in any list status when a valid increment exists. A movie increment sets watch state to `1`, including when release metadata is missing. A series increment advances to the next released regular episode and wraps to the next season when applicable; never advance into unreleased episodes or beyond the available released catalogue. Hide the control when no increment is possible, without an extra limit message. Preserve accepted status and Completed-priority rules.
- On touch devices, quick rating uses the star scale rather than a separate manual score selector. Holding/dragging previews the half-star value; releasing or tapping confirms. Preserve the current rating-mode confirmation, keyboard access, and reduced-motion behavior.
- The list editor must retain and reuse the owner list response, which already contains its currently used fields and artwork, instead of fetching the title on every opening. Open only when the complete editor content is ready and use a restrained fade/scale entrance rather than a blank dialog followed by snapping fields. Avoid duplicating full entry JSON into every row. Apply the same data-ready motion pattern to future list editors; saves still require a network connection.

### Motion, offline browsing, artwork, and layout

- Plan loading by surface rather than showing a full-page placeholder for every data request: card-shaped skeletons for cards (using [the supplied AniList Browse image](references/anilist-browse-card-skeleton-2026.png) as visual reference), appropriate text/row placeholders, fast per-item fades, and an optional thin top progress indicator that never shifts layout. Suppress loading chrome for instant results; when a request genuinely takes time, keep its placeholder stable long enough to avoid flicker. Fast search need not show transient “Searching” or empty result cards while a request is in flight; preserve accessible settled no-match/error feedback.
- Retain as much safely displayable read-only page/data content as practical for navigation within an already-open AnyList tab, including previously loaded lists, title/editor data, Browse, profiles/Social, Stats, Notifications, and Connection Settings. A first load or browser refresh without internet may show the browser's normal network error; do not require an offline-first app shell. Keep private cached data isolated by AnyList user and clear it on explicit logout or account switch. Never cache credentials or tokens. Show available cached content first, use localized loading placeholders for missing data, and show “No internet access” in the affected region once a fetch is confirmed impossible. Offline edits are not queued; attempted edits give an unobtrusive toast, not a Notification. A permanent global offline banner is not required.
- Restore TMDB's ordinary `poster_path` as primary across catalogue, lists, Library, Browse, and details, for existing and newly enriched titles. The earlier language-neutral/textless selection is no longer the primary preference. Retain layout-appropriate crops, but investigate serving poster sizes appropriate to compact previews, ordinary cards, and large detail/Browse artwork rather than fetching one size everywhere.
- Keep white text on blue primary buttons during hover/focus and reduce the blue-background color change. Fix the rightward shift when navigating to Social from another profile view and the shift-and-return on movie/series list navigation. Align available/connection status in mobile Settings without clipping or shifting neighboring controls.

### Required local research and handoff evidence

- Inspect the observable network/loading path of the supplied public AniList list page, including whether assets or list data are prefetched/cached. Do not claim its authenticated editor or offline internals were verified without access; adopt only techniques that fit AnyList's server-first architecture and are proportionate to their complexity.
- Produce an Astro-specific performance report and a visualization of page, API/data, poster-image, browser HTTP, service-worker, and optional server-side image caching paths. Explain CPU, memory and network costs, current image sizes/reuse, where caching currently exists, and where it does not. Include a separate critique and proposed improvements only for critical or major findings.
- Before any VPS verification, later authorized local work must cover focused backend/frontend regressions, production build, desktop and mobile browser acceptance for the changes above, multi-provider overlap/conflict/correction/failure scenarios, offline in-tab navigation and logout isolation, and accessibility/reduced-motion behavior. Existing deployed, real-provider, Android/iPhone, CI and owner-approval gates remain open afterward.

## Implementation progress

### 2026-09-22 local handoff for the additional contract

- Streaming Library membership is a local owner choice with durable per-connection delivery, prompt delivery after save, per-destination retry, and Notifications for remote failures. The title button updates in place; the no-connection choice remains visible. Separate AnyList users may connect the same external identity independently.
- Scheduled overlapping pulls now form a bounded per-user cycle. Complete snapshots are compared against approved source baselines; accepted deltas propagate to eligible destinations and unresolved conflicts remain in Notifications. Stremio modification evidence, Nuvio ordered observations, playback position and watch/rating baseline tests cover newer lower positions, simultaneous ties, source echoes, partial snapshots, and unrelated safe changes. Connection failures create recoverable attention. Local tracked-list and direct-rating changes dispatch after commit; confirmed list watch rollbacks use reviewed provider correction paths.
- List progress, touch stars, ready-to-open editor, ordinary responsive posters, button contrast and layout corrections were implemented in separate commits. Browse has scoped card skeletons and settled empty/error behavior; Fast search uses quiet progress. In-tab navigation retains safe user-partitioned pages and a redacted Connection Settings summary, clears private content on logout/account switch, and blocks offline writes with a toast.
- The [Astro performance report](STAGE-TWO-ASTRO-PERFORMANCE.md) includes the public AniList network observations, request/cache-path diagram and major findings. It does not claim authenticated AniList or offline internals.
- Local proof: disposable PostgreSQL `mt015`, 1,167 backend tests passed, Astro production build passed, and the final pushed implementation head [`f3b7c7b`](https://github.com/antipixelhd/AnyList/actions/runs/35680798143) passed public backend/frontend CI. Authenticated Chromium checks at 320 px found no horizontal overflow on Home, Browse, List, Stats, Notifications, Settings or Title. In-tab offline List return and logout cache purge passed. The populated list editor opened under reduced motion with its fields ready and zero animation/transition duration. Focused regressions cover no-connection Library, partial delivery, overlap/conflict/failure, media-server first import, rating changes and rollback fan-out. Owner manual review, real connected providers, physical Android/iPhone and VPS work remain open.
...
### Historical textless poster preference (superseded on 2026-09-21)
...
Implementation progress: the Notifications API projects unresolved streaming/cloud actions as one title group with per-service state and error detail, and the page renders one card per title. Applied/cancelled actions and resolved deletion reviews are excluded; deletion markers keep outstanding services visible through partial success. After Docker Desktop was restored, the PostgreSQL-backed multi-provider success/partial-failure/retry/final-removal regression passed. A disposable two-provider preview fixture rendered one card with both provider rows and the error at 390px without overflow; the misleading “You're all caught up” state was removed while delivery cards remain. Final deployed-build and live-provider verification remain open.
...
Visual-pass progress: a measured first-viewport pass aligns the avatar with the navigation edge and filter rail, narrows the profile navigation, widens the list column, and condenses desktop rows to 52px while retaining 44px square artwork in 54px phone rows. Stats presents its six headline metrics as an icon-led ledger with separators and keeps charts in open sections rather than repeated cards. `UI-REFERENCE.md` records the reproducible geometry and the live AniList row/Stats rendering limitation. The final local cross-page review covered Home, Browse, Profile/List, Stats and Title at 320×720, 390×844, 1440×900 and 3840×2160; all retained their hierarchy without horizontal overflow. Public deployment, real-device checks and owner acceptance remain open.
...
- Library implementation handoff (read-only code inspection, 2026-09-21): `frontend/src/pages/title/[id].astro` currently has only the Favorite action. `backend/routers/tracking.py` exposes an owner-only, read-only `/library` projection of Stremio/Nuvio `CollectionFile` rows. `backend/models/collection.py` stores one `Collection` per user/title with multiple provider `CollectionFile` rows. The inherited manual `/collect` and `DELETE /collect` endpoints in `backend/routers/media.py` mutate that aggregate and invoke `_push_collection_change`, but addition returns early whenever any Collection already exists, removal deletes the whole aggregate, and neither response reports per-provider delivery. Do not connect the new button to those endpoints unchanged: first define how an owner-requested library state coexists with observed provider membership, how each eligible Stremio/Nuvio write is retried, and how pending/failed/succeeded states remain truthful across refreshes and partial delivery. The observed-provider `_fan_out_streaming_library_changes` in `backend/routers/sync.py` excludes a source connection and catches destination errors, so its current contract also cannot by itself prove a local button action reached every provider.
...
Login progress: the sign-in route now has a full-width AnyList navy/blue shell, quiet public navigation, restrained hero, matching form surfaces and responsive layout. Its existing password, conditional OIDC, 2FA, recovery and bootstrap-restore paths remain wired; local invalid-password handling and desktop/phone rendering were checked. Public Google and 2FA interaction on the final deployed build still need verification. `LEGACY-UI-INVENTORY.md` records the remaining Scrob-style route families; numeric profile and Stats presentations are replaced by privacy-aware canonical redirects, while broader legacy-page retirement remains open.

Legacy-page progress: the inherited Movies, Shows, Search, Discover, Trending and Airing Today presentations are replaced by compatibility redirects to canonical Browse. Movie/Series type and title queries are retained where meaningful; signed-in user search bookmarks route to the canonical Social profile search. Login navigation now links directly to Browse. The non-visual `discover-title` import bridge remains because Browse and external-ID compatibility routes use it to resolve remote catalogue results. Custom-list, collection, progress, dropped, history, calendar, media/show/episode detail, people/company, profile-see-all and playback-dashboard presentations now redirect into canonical List, Library, Profile, Home, Browse or Title workflows. TMDB/TVDB identities are resolved before navigation and are never treated as AnyList IDs. Status deep links select the intended canonical list filter. Stored data, backend APIs and integration routes remain intact. Required administrative, authentication and protocol surfaces remain available without presenting a second movie/series UI.

Library-action progress: title details now present separate Favorite and Library controls. Migration `mt015` stores the owner's desired membership independently of observed collection rows and records pending/applied delivery per opted-in Stremio/Nuvio connection. Provider writes wait for the reviewed reconciliation baseline, retain failures and safe error names for retry, retry from the title page and later connection pulls, and appear in the grouped pending-connection queue. A successful write alone updates that provider's observed membership. Explicit removal remains authoritative during later full pushes even when another source still owns a collection row. Focused PostgreSQL regressions cover no-target refusal, baseline gating, partial failure, manual retry, provider-preserving removal, grouped queue cleanup, and prevention of re-adding an explicitly removed title. Local desktop and 390×844 browser checks confirmed the distinct controls and truthful pending explanation without overflow. Final deployed writes to disposable real Stremio/Nuvio accounts remain open.
