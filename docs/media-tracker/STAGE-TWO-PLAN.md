...
Legacy-page progress: the inherited Movies, Shows, Search, Discover, Trending and Airing Today presentations are replaced by compatibility redirects to canonical Browse. Movie/Series type and title queries are retained where meaningful; signed-in user search bookmarks route to the canonical Social profile search. Login navigation now links directly to Browse. The non-visual `discover-title` import bridge remains because Browse uses it to resolve remote catalogue results. The inherited custom-list, collection, progress, dropped, history and calendar pages are also thin redirects into the canonical username list, Library, Profile or Home workflows. Status deep links select the intended canonical list filter. Stored custom-list data and backend APIs remain intact. Old detail/people pages and administrative flows still require the inventory's workflow-by-workflow assessment.
