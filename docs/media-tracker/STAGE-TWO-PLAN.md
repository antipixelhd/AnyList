...
Legacy-discovery progress: the inherited Movies, Shows, Search, Discover, Trending and Airing Today presentations are replaced by compatibility redirects to canonical Browse. Movie/Series type and title queries are retained where meaningful; signed-in user search bookmarks route to the canonical Social profile search. Login navigation now links directly to Browse. The non-visual `discover-title` import bridge remains because Browse uses it to resolve remote catalogue results. Personal collections, history/calendar, old detail/people pages and administrative flows still require the inventory's workflow-by-workflow assessment.
