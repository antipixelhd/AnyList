...
| Isolated test deployment | Public code head `30f5da6` is on the isolated test instance. The app and retained database are healthy at `mt014`; login and PWA assets passed HTTPS checks, and the earlier read-only audit confirmed disposable Stremio/Nuvio push flags remain off. | Complete final Google-authenticated interaction checks on this deployed build and record them in [issue #2](https://github.com/antipixelhd/AnyList/issues/2). |
