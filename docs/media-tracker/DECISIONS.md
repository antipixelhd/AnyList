...
Netflix CSV import uses four steps: prepare, resolve uncertain title identities, review show progress, and finalize. Cancel is available throughout. The import changes personal state only after the final Import action. Uncertain show and movie identities require an explicit confirm, remap, or skip. Episode titles do not require review: exact matches anchor an automatic guess based on the export's viewing order and the released catalogue.

Netflix season labels can split a TMDB aired season into multiple parts. When the labelled TMDB season is absent, search the show's available seasons by exact episode title and accept only a unique match. A 404 for one season does not abort the CSV; observations without a safe released position are discarded and counted. Other provider failures remain visible.

Netflix movie rows have no release year. Multiple exact-title TMDB films may resolve automatically only when one pre-watch release has at least 1,000 votes, at least four times the combined votes of the other same-title candidates, and at least 1.5 times the runner-up popularity. Close remakes stay in review. Search longer colon-bearing TV title prefixes and use the longest exact catalogue title, while preserving the full suffix as a possible episode title. Guessed episode positions never strengthen show-identity confidence. Rows consisting solely of `: Episode N` lack a show title and are excluded before matching, with a separate count in the import summary.

Progress review shows incompletely represented shows first, sorted by title with posters. Missing episode positions are filled from earlier and later exact anchors in viewing order; trailing observations can continue into the next released season. Repeated watches retain their dates without advancing twice. A bounded observation with no free distinct position counts as covered; an unbounded observation with no safe released position is discarded. Incompletely represented shows default to Partial at the highest exact or guessed episode; only fully represented released seasons default to Completed. Partial offers Watching, Paused, or Dropped, defaulting to Watching. Selected progress is cumulative; lowering its endpoint excludes later CSV rows. Existing progress never decreases. Automatic suggestions preserve existing manual statuses, while an explicit review status change overrides them. Skip leaves the title untouched.

The importer opens its review workflow in a wide dialog. Show progress follows title review through Continue and Back, before a final review containing only the selected movies, shows, and episode-position totals. Completed fills every numbered position within each season's known released count, even if TMDB omitted an episode record or air date; those positions become provisional watches without invented viewing dates.

Accepted viewing dates set the earliest show start date and, for Completed shows, the latest show finish date. A movie watch sets its finish date. Existing nonempty dates remain authoritative. Guessed positions retain their CSV watch dates; provisional episodes added only by cumulative progress have unknown watch dates. Reimports deduplicate historical watches. English and German exports are in scope; the import harness should allow later providers such as Disney+ and Prime without committing to their formats now.
